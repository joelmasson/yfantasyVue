export function teamNamesAbv(name: string) {
  switch (name) {
    case "Atlanta Flames":
      return "AFM";
    case "Anaheim Ducks":
      return "ANA";
    case "Arizona Coyotes":
      return "ARI";
    case "Atlanta Thrashers":
      return "ATL";
    case "Boston Bruins":
      return "BOS";
    case "Brooklyn Americans":
      return "BRK";
    case "Buffalo Sabres":
      return "BUF";
    case "Carolina Hurricanes":
      return "CAR";
    case "Columbus Blue Jackets":
      return "CBJ";
    case "Bay Area Seals/California Golden Seals":
      return "CGS";
    case "Calgary Flames":
      return "CGY";
    case "Chicago Blackhawks":
      return "CHI";
    case "Cleveland Barons":
      return "CLE";
    case "Colorado Rockies":
      return "CLR";
    case "Colorado Avalanche":
      return "COL";
    case "Dallas Stars":
      return "DAL";
    case "Detroit Cougars":
      return "DCG";
    case "Detroit Red Wings":
      return "DET";
    case "Detroit Falcons":
      return "DFL";
    case "Edmonton Oilers":
      return "EDM";
    case "Florida Panthers":
      return "FLA";
    case "Hamilton Tigers":
      return "HAM";
    case "Hartford Whalers":
      return "HFD";
    case "Kansas City Scouts":
      return "KCS";
    case "Los Angeles Kings":
      return "LAK";
    case "Minnesota Wild":
      return "MIN";
    case "Montreal Maroons":
      return "MMR";
    case "Minnesota North Stars":
      return "MNS";
    case "Montreal Canadiens":
      return "MTL";
    case "Montreal Wanderers":
      return "MWN";
    case "New Jersey Devils":
      return "NJD";
    case "Nashville Predators":
      return "NSH";
    case "New York Americans":
      return "NYA";
    case "New York Islanders":
      return "NYI";
    case "New York Rangers":
      return "NYR";
    case "California/Oakland Seals":
      return "OAK";
    case "Ottawa Senators":
      return "OTT";
    case "Philadelphia Flyers":
      return "PHI";
    case "Phoenix Coyotes":
      return "PHX";
    case "Pittsburgh Pirates":
      return "PIR";
    case "Pittsburgh Penguins":
      return "PIT";
    case "Quebec Bulldogs":
      return "QBD";
    case "Philadelphia Quakers":
      return "QUA";
    case "Quebec Nordiques":
      return "QUE";
    case "Seattle Kraken":
      return "SEA";
    case "Ottawa Senators (original)":
      return "SEN";
    case "St. Louis Eagles":
      return "SLE";
    case "San Jose Sharks":
      return "SJS";
    case "St. Louis Blues":
      return "STL";
    case "Toronto Hockey Club/Toronto Arenas":
      return "TAN";
    case "Tampa Bay Lightning":
      return "TBL";
    case "Toronto Maple Leafs":
      return "TOR";
    case "Toronto St. Patricks":
      return "TSP";
    case "Utah Hockey Club":
      return "UTA";
    case "Vancouver Canucks":
      return "VAN";
    case "Vegas Golden Knights":
      return "VGK";
    case "Winnipeg Jets (original)":
      return "WIN";
    case "Winnipeg Jets":
      return "WPG";
    case "Washington Capitals":
      return "WSH";
    default:
      break;
  }
}
